...
// rowCountExpectation checks the number of rows. Its Result.Name appends the
// observed count; per-row Result fields are left empty.
...
// RowCount builds a table-level expectation from a row-count predicate.
...
func RowCountBetween[T any](lo, hi int) Expectation[T] {
	return RowCount[T](fmt.Sprintf("row count in [%d,%d]", lo, hi), func(n int) bool {
		return n >= lo && n <= hi
	})
}
...
func RowCountEqual[T any](want int) Expectation[T] {
	return RowCount[T](fmt.Sprintf("row count == %d", want), func(n int) bool {
		return n == want
	})
}
